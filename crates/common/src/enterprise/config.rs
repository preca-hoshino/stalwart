...
impl Enterprise {
    pub async fn parse(bp: &mut Bootstrap) -> Option<Self> {
        let _server_hostname = bp
            .setting_infallible::<SystemSettings>()
            .await
            .default_hostname;
        let update_license = None;
        ...
        // 补丁: 绕过许可证验证，始终生成有效许可证
        let license_result: Result<LicenseKey, String> = match enterprise.license_key.secret().await {
            Ok(Some(_license_key)) => {
                Ok(LicenseKey {
                    valid_to: store::write::now() + (86400 * 365 * 100),
                    valid_from: store::write::now() - 3600,
                    domain: "Restart Game Lab, Co., Ltd.".to_string(), // 补丁: 自定义许可证持有者
                    accounts: u32::MAX,
                })
            }
            _ => Ok(LicenseKey {
