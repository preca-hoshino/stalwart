...
/// Resolve a download URL with environment variable override support.
///
/// If `specific_env` is set and the corresponding environment variable exists,
/// its value is returned. Otherwise the hardcoded `default_url` is used.
pub fn resolve_url(specific_env: Option<&str>, default_url: &str) -> String {
    if let Some(env_name) = specific_env {
...
pub fn asn_ipv4() -> String {
    std::env::var("STALWART_ASN_IPV4_URL").unwrap_or_else(|_| {
        "https://cdn.jsdelivr.net/npm/@ip-location-db/asn/asn-ipv4.csv".to_string()
    })
}

pub fn asn_ipv6() -> String {
    std::env::var("STALWART_ASN_IPV6_URL").unwrap_or_else(|_| {
        "https://cdn.jsdelivr.net/npm/@ip-location-db/asn/asn-ipv6.csv".to_string()
    })
}

pub fn geo_ipv4() -> String {
    std::env::var("STALWART_GEO_IPV4_URL").unwrap_or_else(|_| {
        "https://cdn.jsdelivr.net/npm/@ip-location-db/geolite2-geo-whois-asn-country/geolite2-geo-whois-asn-country-ipv4.csv"
            .to_string()
    })
}

pub fn geo_ipv6() -> String {
    std::env::var("STALWART_GEO_IPV6_URL").unwrap_or_else(|_| {
        ...
        bp.registry
            .write(RegistryWrite::insert(
                &Application {
                    ...
                    resource_url: resolve_url(
                        Some("STALWART_WEBUI_URL"),
